...
3. If any of the three spec families are missing or clearly stale for the request, run the intent of `author-specs` first so planning has durable source files.
4. Run the intent of `office-hours`.
5. Run the intent of `plan-ceo-review`.
6. Run the intent of `plan-eng-review`.
7. Run the intent of `plan-design-review` when the work is user-facing.
8. Run the intent of `plan-workstreams` to build the sprint index and subplans from the resulting docs plus `.ai/DECISIONS/`.
