...
4. As soon as the change is coherent, run the intent of `validate-change` instead of stopping at a raw diff. Do not treat implementation alone as a completed result.
5. If validation finds actionable issues, apply the smallest credible fix and re-run `validate-change`.
