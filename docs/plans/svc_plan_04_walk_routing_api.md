...
| RouteOption | `SAFE`, `SHORTEST` | `SAFE_WALK`, `FAST_WALK`, `ACCESSIBLE_TRANSIT` |
...
public enum RouteOption { SAFE_WALK, FAST_WALK, ACCESSIBLE_TRANSIT }
```
...
public class RoutingProfileResolver {
    ...
    private static final Map<DisabilityType, Map<RouteOption, String>> PROFILE_MAP = Map.of(
        DisabilityType.VISUAL, Map.of(
            RouteOption.SAFE_WALK, "visual_safe",
            RouteOption.FAST_WALK, "visual_fast"
        ),
        DisabilityType.MOBILITY, Map.of(
            RouteOption.SAFE_WALK, "wheelchair_safe",
            RouteOption.FAST_WALK, "wheelchair_fast"
        )
    );
...
public class RouteSearchService {
    ...
    public RouteSearchResponse search(RouteSearchRequest req) {
        List<RouteOption> options = req.routeOptions() != null
            ? req.routeOptions()
            : List.of(RouteOption.SAFE_WALK, RouteOption.FAST_WALK, RouteOption.ACCESSIBLE_TRANSIT);
    ...
    private RouteOptionResult executeOption(RouteSearchRequest req, RouteOption option) {
        return switch (option) {
            case SAFE_WALK  -> walkRoutingService.route(req.disabilityType(), SAFE_WALK,
                                   req.startPoint(), req.endPoint());
            case FAST_WALK  -> walkRoutingService.route(req.disabilityType(), FAST_WALK,
                                   req.startPoint(), req.endPoint());
            case ACCESSIBLE_TRANSIT -> transitService.orchestrate(req);  // svc_plan_06
        };
    }
}
```
...
public record RouteSearchRequest(
    ...
    List<RouteOption> routeOptions  // null 허용 (전체 반환)
) {}
...
- [ ] `POST /routes/search`: SAFE_WALK + FAST_WALK 병렬 응답
...
- [ ] `ACCESSIBLE_TRANSIT` → stub(`available: false, reason: NOT_IMPLEMENTED`)
- [ ] 단위/통합 테스트 통과
