...
└── src/
    ...
    │   ├── RouteOptionTabs.tsx       # SAFE_WALK / FAST_WALK / ACCESSIBLE_TRANSIT 탭
...
export type RouteOption    = 'SAFE_WALK' | 'FAST_WALK' | 'ACCESSIBLE_TRANSIT';
...
export interface RouteOptionResult {
    ...
    segments?: SegmentDto[];
    legs?: LegDto[];           // ACCESSIBLE_TRANSIT
    markers?: MarkerDto[];     // ACCESSIBLE_TRANSIT 정류장/역 마커
}
...
export default function App() {
    ...
    const [activeOption, setActiveOption] = useState<RouteOption>('SAFE_WALK');
...
- [ ] SAFE_WALK / FAST_WALK 탭 전환 → 지도 경로 변경
- [ ] ACCESSIBLE_TRANSIT 탭 → 정류장/역 마커 지도 표시 (실제 정류장 위치)
